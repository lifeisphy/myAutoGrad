...
mid_size = 128
W1 = t.make_param([0 for _ in range(input_size)], [input_size, mid_size])
b1 = t.make_param([0 for _ in range(mid_size)], [mid_size])
W2 = t.make_param([0 for _ in range(mid_size)], [mid_size, 10])
b2 = t.make_param([0 for _ in range(10)], [10])
layer1 = t.relu(t.mul(W1, output,0,0) + b1 )
